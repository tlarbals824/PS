    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < nmk[1] + nmk[2]; i++) {
            ...
            if (command[0] == 1) {
                ...
                bw.write(sum(tree, 1, 1, nmk[0], command[1], command[2]) + "\n");
            }
        }
        bw.flush();
    }
    ...
    static long init(long[] tree, long[] arr, int idx, int start, int end) {
        if (start == end) {
            ...
            return tree[idx] = init(tree, arr, idx * 2, start, (start + end) / 2)
                    + init(tree, arr, idx * 2 + 1, (start + end) / 2 + 1, end);
        }
    }
        ...
        if (start == end) {
            ...
            if ((start + end) / 2 >= changeIdx) {
                return tree[idx] = tree[idx * 2 + 1]
                        + update(tree, idx * 2, start, (start + end) / 2, changeIdx, changeValue);
            } else {
                return tree[idx] = tree[idx * 2]
                        + update(tree, idx * 2 + 1, (start + end) / 2 + 1, end, changeIdx, changeValue);
            }
        }
    }

    static long sum(long[] tree, int idx, int start, int end, long b, long c) {
        if (end < b || start > c)
            return 0;
        else if (start >= b && c >= end) {
            return tree[idx];
        } else {
            return sum(tree, idx*2, start, (start + end) / 2, b, c)
                    + sum(tree, idx*2+1, (start + end) / 2 + 1, end, b, c);
        }
    }
